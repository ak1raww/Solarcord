...
import type { MessageSnapshot, SentryEvent, SentryEventType, SentryScope, VoiceParticipant, VoiceState, VoiceStateFlag } from "./types";

const SETTINGS_ENTRY_KEY = "solarcord_sentry";
...
const SentryTab = LazyComponent(() => require("./components/SentryTab").default);
...
export const settings = definePluginSettings({
    ...
    addContextMenu: {
        ...
        default: true,
        description: "Add a Sentry toggle to user context menus.",
    },
    ignoreBots: {
        type: OptionType.BOOLEAN,
        default: false,
    ...
    notifyEvents: {
        type: OptionType.BOOLEAN,
        default: false,
        description: "Send notifications for high signal sentry events.",
    },
    trackSelf: {
        type: OptionType.BOOLEAN,
        default: false,
...
const getScope = (userId: string, guildId?: string): SentryScope | undefined => {
...
const getGuildInfo = (guildId: string | undefined): Pick<SentryEvent, "guildId" | "guildName"> => {
...
const notify = (event: SentryEvent) => {
...
const addEvent = (entry: Omit<SentryEvent, "id" | "timestamp">) => {
...
const addUserEvent = (type: SentryEventType, userId: string, details: string, extra: Partial<SentryEvent> = {}) => {
...
const addServerEvent = (type: SentryEventType, guildId: string | undefined, details: string, extra: Partial<SentryEvent> = {}) => {
...
const patchUserContext: NavContextMenuPatchCallback = (children, { user }: UserContextProps) => {
    ...
    group.splice(index, 0,
        <Menu.MenuItem
            id="vc-sentry-toggle"
            label={tracked ? "Remove from Sentry" : "Add to Sentry"}
            action={() => {
...
export default definePlugin({
    name: "SolarSentinel",
    description: "Monitor users/servers through a dashboard for moderation purposes.",
    ...
    toolboxActions: {
        "Open Sentry": () => SettingsRouter.openUserSettings(`${SETTINGS_ENTRY_KEY}_panel`),
    },
