...
import type { SentryEvent, SentryEventType, VoiceParticipant } from "../types";

type EventFilter = "all" | "activity" | "message" | "presence" | "reaction" | "server" | "typing" | "voice";
type SentryPage = "user" | "server";
...
const cl = classNameFactory("vc-sentry-");
...
const pageOptions: Array<{ label: string; value: SentryPage; }> = [
    { label: "User Sentry", value: "user" },
    { label: "Server Sentry", value: "server" },
];

const typeLabels: Record<SentryEventType, string> = {
...
const getBadgeClass = (type: SentryEventType) => {
...
const eventMatchesPage = (event: SentryEvent, page: SentryPage) =>
    page === "server" ? event.scope === "server" : event.scope !== "server";

const eventMatchesFilter = (event: SentryEvent, filter: EventFilter) => {
...
const eventMatchesQuery = (event: SentryEvent, query: string) => {
...
const EventDetailsModal = ErrorBoundary.wrap(function EventDetailsModal({ event, modalProps }: { event: SentryEvent; modalProps: RenderModalProps; }) {
...
const openEventModal = (event: SentryEvent) => {
...
function EventRow({ event }: { event: SentryEvent; }) {
    const channel = event.channelId ? ChannelStore.getChannel(event.channelId) : undefined;
    const guild = event.guildId ? GuildStore.getGuild(event.guildId) : undefined;
    const location = [
...
function SentryTab() {
    const [events, setEvents] = useState<SentryEvent[]>(getEvents());
    const [page, setPage] = useState<SentryPage>("user");
    ...
    const [collapsedPages, setCollapsedPages] = useState<Record<SentryPage, boolean>>({
        server: false,
        user: false,
    });
    ...
    const pageHeading = page === "server" ? "Server Sentry" : "User Sentry";
    ...
    const copyEvents = () => {
        try {
            void Promise.resolve(copyToClipboard(JSON.stringify(filteredEvents, null, 2))).then(
                ...
                () => toast("Failed to copy sentry events.", Toasts.Type.FAILURE)
            );
        } catch {
            toast("Failed to copy sentry events.", Toasts.Type.FAILURE);
        }
        ...
        <SettingsTab>
            <div className={cl("root")}>
                <div className={cl("header")}>
                    <HeadingPrimary>Sentry</HeadingPrimary>
                    <div className={cl("actions")}>
...
export default wrapTab(SentryTab, "Sentry");
